...
const modalSource = computed(() => props.modalKey ? petSkillData[props.modalKey] : undefined)
...
<template>
  <div class="rounded-md bg-black/30 p-3">
    <div class="grid grid-cols-3 gap-3 py-[13px]">
      <div class="col-span-1 flex flex-col items-center">
        <button type="button" class="group w-full" @click="openModal" aria-label="보조기 상세 열기">
          <div
            :class="petBoxClass || 'w-full max-w-[120px] aspect-square overflow-hidden rounded-md border-2 border-red-500'"
            class="relative"
          >
            <img :src="petImg" :class="['w-full h-full', fitClass, petImgClass]" :style="imgStyle" />
            <span class="absolute bottom-1.5 right-1.5 text-[10px] px-1.5 py-0.5 rounded bg-black/60 text-white/85 border border-white/20 opacity-0 group-hover:opacity-100 transition">
              상세 보기
            </span>
          </div>
        </button>
        ...
        <span
          :class="[
            ...
            ((petName || '').trim().length > 6) ? 'text-sm md:text-[12px]' : (petNameClass || 'text-sm md:text-sm')
          ]"
        >{{ petName }}</span>
      </div>
      ...
      <div class="col-span-2 flex flex-col justify-center">
        <div class="text-center text-xs md:text-[12px] text-white/90 mb-2 md:mb-3">
          <span class="inline-flex items-center rounded-full px-3 py-1 text-xs md:text-[12px] bg-white/10 text-white border border-white/20 backdrop-blur-[2px] shadow">
            {{ title ?? '보조기 스킬 추천' }}
          </span>
        </div>
  ...
  <transition name="pet-fade">
    <div v-if="modalOpen" class="fixed inset-0 z-[90]" aria-modal="true" role="dialog">
      <div class="absolute inset-0 bg-black/70 backdrop-blur-[2px]" @click="closeModal"></div>

      <div class="absolute inset-0 grid place-items-center p-4">
        <div class="w-full max-w-2xl rounded-xl bg-[#14141C] text-white shadow-2xl border border-white/10 animate-[petIn_200ms_ease-out]">
          <!-- 헤더 -->
          <div class="flex items-center gap-3 p-4 border-b border-white/10">
            <div class="h-[60px] w-[60px] rounded-md overflow-hidden border border-white/15">
              <img :src="petImg" class="w-full h-full object-cover" />
            </div>
            <div class="flex-1">
              <div class="text-[17px] md:text-lg font-bold text-white/95">{{ petName }}</div>
              <div class="mt-1 flex flex-wrap gap-1">
                <span v-if="modalLevel !== undefined" class="inline-flex items-center rounded-md px-2 py-0.5 text-[11px] bg-white/10 border border-white/20">
                  Level {{ modalLevel }}
                </span>
              </div>
            </div>
            <button class="ml-2 mb-6 text-white/80 hover:text-white rounded-md p-1.5 hover:bg-white/10 transition" @click="closeModal" aria-label="닫기">✕</button>
          </div>
          ...
          <div class="p-4 md:p-5 space-y-3 max-h-[70vh] overflow-y-auto">
            ...
            >
              <div class="h-[60px] w-[60px] shrink-0 rounded-md bg-white/[0.06] border border-white/15 grid place-items-center">
                <img :src="b.img" class="w-[50px] h-[50px] object-contain opacity-90" alt="" />
              </div>
              ...
              <div class="flex-1">
                <div class="flex items-center gap-2 flex-wrap">
                  ...
                  <!-- 뱃지 -->
                  <span class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px]" :class="pillClass(b.kind)">
                    {{ b.kind === 'active' ? '액티브' : '패시브' }}
                  </span>
                  <span v-if="b.level !== undefined"
